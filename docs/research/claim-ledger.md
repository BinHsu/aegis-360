...
| Apple's `VNTrackObjectRequest` can discover an arbitrary bicycle without an initial observation. | disproven | Apple defines it as tracking a previously identified object. A detector or supplied initial box must seed it. [VNTrackObjectRequest](https://developer.apple.com/documentation/vision/vntrackobjectrequest) |
| A Core ML object detector can seed Apple's Vision object tracker. | confirmed | Apple's object-recognition sample returns `VNRecognizedObjectObservation` boxes and explicitly describes using them to initialize `VNTrackObjectRequest`. [Recognizing Objects in Live Capture](https://developer.apple.com/documentation/vision/recognizing-objects-in-live-capture) |
| Apple currently hosts Core ML YOLOv3 variants suitable for an offline bootstrap. | confirmed | Apple's model gallery lists full and Tiny YOLOv3 variants, including an 8.9 MB Tiny Int8LUT model. Model license and checksum still require inspection at acquisition. [Core ML model gallery](https://developer.apple.com/machine-learning/models/) |
| Ultralytics can be treated as an Apache-2.0 dependency of this repository. | disproven | The official repository is AGPL-3.0 with a separate enterprise-license option. Keep it isolated unless a deliberate licensing decision is made. [Ultralytics license](https://github.com/ultralytics/ultralytics/blob/main/LICENSE) |
| YOLOX is a permissively licensed detector alternative. | confirmed | The official YOLOX repository declares Apache-2.0 and provides ONNX-oriented deployment paths. Core ML conversion and output equivalence remain unverified here. [YOLOX](https://github.com/Megvii-BaseDetection/YOLOX) |
| YOLOX-Tiny is the lowest-risk next conversion experiment for the current 416×416 viewport contract. | supported inference | The official table reports 416×416 input, 5.06M parameters, 6.45 GFLOPs and 32.8 COCO mAP, with official release weights and ONNX export. This supports a bounded feasibility experiment, not benchmark recall or Core ML compatibility. [YOLOX](https://github.com/Megvii-BaseDetection/YOLOX), [official ONNX deployment](https://github.com/Megvii-BaseDetection/YOLOX/tree/main/demo/ONNXRuntime) |
| General PyTorch support in Core ML Tools proves YOLOX detection conversion will work. | unverified | Apple documents traced/exported PyTorch conversion, but that does not establish YOLOX decode or NMS equivalence. A numerical conversion gate is required. [Core ML Tools PyTorch conversion](https://apple.github.io/coremltools/docs-guides/source/convert-a-torchvision-model-from-pytorch.html) |
