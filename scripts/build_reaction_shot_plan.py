...
def main() -> int:
    ...
    for name in ("grid_json", "roles_json", "reactions_json", "availability_json", "gain_json", "output_json"):
        parser.add_argument(name, type=Path)
    args = parser.parse_args()
    inputs = [args.grid_json, args.roles_json, args.reactions_json, args.availability_json, args.gain_json]
    ...
    artifact = build_reaction_plan(load(args.grid_json), load(args.roles_json), load(args.reactions_json), load(args.availability_json), load(args.gain_json), grid_sha256=hashlib.sha256(grid_bytes).hexdigest())
