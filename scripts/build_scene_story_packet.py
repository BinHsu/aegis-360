#!/usr/bin/env python3
"""Build one atomic local-story review packet for a neutral scene event."""
...
from aegis360.scene_story_packet import build_scene_story_packet  # noqa: E402
...
def main() -> int:
    ...
    if not args.timeline_json.is_file() or not args.grid_json.is_file():
        parser.error("timeline or grid is missing")
    if args.output_json.exists():
        parser.error("refusing to overwrite output")
    timeline_raw = args.timeline_json.read_bytes()
    grid_raw = args.grid_json.read_bytes()
    packet = build_scene_story_packet(
        json.loads(timeline_raw), json.loads(grid_raw), event_id=args.event_id,
    ...
    print(json.dumps({"sample_count": len(packet["samples"])}, sort_keys=True))
    return 0
